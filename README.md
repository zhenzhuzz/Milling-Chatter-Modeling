...
| 西门子 LMS 力锤激振实验 | 实验近景 |
| --- | --- |
| ![](Media/西门子lms_力锤激振实验.jpg) | ![](Media/西门子lms_力锤激振实验_近景.jpg) |

实验结果可为后续**再生效应时滞微分方程**中的刚度、阻尼等参数提供精确输入，也能帮助验证数值模型的可靠性。通过对 FRF 曲线的分析，可以识别出主导振动模态，并结合稳定性叶瓣图预测切削过程中容易发生颤振的转速区间。


### 铣削动力学与再生效应
| 主轴-刀柄-刀具系统示意图 | 铣削过程示意图 |
| --- | --- |
| ![](Media/图3%201主轴-刀柄-刀具系统示意图.png) | ![](Media/图3%202%20铣削过程示意图.png) |

| 铣削过程系统框图 | 工件表面的波浪状 |
| --- | --- |
| ![](Media/图3%203%20铣削过程的系统框图.png) | ![](Media/图3%204%20工件表面的波浪状.png) |

| 相邻刀齿切削痕迹相位差对动态切屑厚度的影响 |
| --- |
| ![](Media/图3%205相邻刀齿切削痕迹相位差对动态切屑厚度的影响.png) |
...
| 自适应主轴调速示意图 |
| --- |
| ![](Media/图4%201%20自适应主轴调速示意图.png) |

| 变转速铣削过程中刀尖振动位移的时域响应 |
| --- |
| ![](Media/图4%203%20变转速铣削过程中刀尖振动位移的时域响应.png) |

| 基于齿周期采样的刀尖振动状态空间图 |
| --- |
| ![](Media/图4%204%20基于齿周期采样的刀尖振动状态空间图.png) |

| 不同主轴转速阶段（4520-8000-10000 rpm）刀尖振动位移的时频分析 |
| --- |
| ![](Media/图4%205%20不同主轴转速阶段（4520-8000-10000%20rpm）刀尖振动位移的时频分析.png) |

---

### 基础振动类型演示（GIF 动画）
这些动画演示了 stick–slip 模型下的三种典型振动现象，帮助理解颤振与自激振动的机理。

- **m034_Free**（自由振动）：无外力，初始位移/速度 → 振幅指数衰减  
![](Media/m034_Free_eraser_stick_slip_animation_Subplot_disp_force.gif)

- **m033_Forced**（受迫振动）：外部周期力驱动，频率＝外力频率；共振时振幅最大  
![](Media/m033_Forced_eraser_stick_slip_animation_Subplot_disp_force.gif)

- **m032_Stick–Slip**（自激振动原理示例）：无外部周期力，系统自带调制机制（弹簧伸缩）将稳定能量源调制为周期力 → 振幅自增至非线性饱和，频率≈固有频率。这与铣削颤振的反馈机理一致，但更易直观理解。  
![](Media/m032_eraser_stick_slip_animation_Subplot_disp_force_variation.gif)
